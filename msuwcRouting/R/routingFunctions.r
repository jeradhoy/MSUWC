...
RouteWater <- function(edges, catchments, Rsurf, Rsub, spinUpCycles=0, debugMode=F, by="day", widthCoeffs=c(.3, .6), manningN=.07, slopeMin=.01, aCoeffCoeff=3){
	...
	for(timeStep in 1:timeLength){
		...
		if(timeStep == 1){
		    ...
		    sRiv <- tail(results$sRiv[, hydroID], n=1)
		    height <- tail(results$h[, hydroID], n=1)
		    sSub<- tail(results$sSub[, hydroID], n=1)
		...
		print(paste("ETA:", 
			    round((Sys.time()-start)*length((timeStep+1):timeLength)/10*(spinUpCycles+1), 2), 
			    "seconds or", 
