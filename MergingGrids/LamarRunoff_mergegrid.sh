...
let counter=1
	...
	do
	cdo mergegrid file$number.nc Subset_1/output_$index/stand_daily_subset_$index.nc file$next.nc
...
let number= #set number to the final number file from Subset_1
let next= #set to whatever $number is set to plus one (do not do let next=$number+1,
#actually type in the integer)
let again= #set to whatever $next is set to plus one
...
cdo setrtomiss,10E4,10E50 file37.nc merged.nc