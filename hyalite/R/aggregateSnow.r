...
AggregateSnow <- function(defaults=setupList, catchmentPolygons,  snowFileName=NULL, msroFileName=NULL, ncDir=NULL, runoffVar=NULL, useWeights=NULL, startDate=NULL, leapDays=F, by="day", fname=NULL){

	## Set variables to default list if not specified
	snowFileName <- if(is.null(snowFileName)) defaults$snowpackNcName
	msroFileName <- if(is.null(msroFileName)) defaults$surfaceNcName
	ncDir <- if(is.null(ncDir)) defaults$ncdir
	runoffVar <- if(is.null(runoffVar)) defaults$surfaceVarName
	useWeights <- if(is.null(useWeights)) defaults$aggregateWithWeights
	startDate <- if(is.null(startDate)) defaults$simStartDate
	by <- if(is.null(by)) defaults$timeStep
    ...
    for(ncFile in 1:length(filesToProcess)){
		...
		runoffbrick <- brick(paste(ncDir, filesToProcess[ncFile], sep=""), varname=runoffVar)
		print("Finished building bricks")

		# Use extract function to sum runnoff for each catchment
		runoff <- data.frame(t(extract(runoffbrick, catchmentPolygons, weights=useWeights, na.rm=T, fun=mean)))
		rm(runoffbrick)
