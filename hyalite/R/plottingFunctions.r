	...
	for(i in 1:length(gauges)){
		...
		for(season in 1:4){
		   ...
		   next
		    ...
		    print("Plotting Seasonal")
		    plot(seq(as.Date(simStartDate), length.out=length(flow1), by="year"), flow1, type="l", lwd=2, cex=1.3, cex.lab=1.4,  cex.axis=1.3, col="red", xlab="",  ylab=expression(paste("Flow (m"^"3", "/s)")), ylim=c(0, max(c(max(flow1, na.rm=T), max(gaugeFlow, na.rm=T)))))
		    print("Guage data")

		    lines(seq(as.Date(simStartDate), length.out=length(gaugeFlow), by="year"), gaugeFlow, lwd=2)
		    lines(gaugeFlow)
		    print("Plotting Seasonal")
	    ...
	    print("Plotting Monthly")
                ...
                lines(as.Date(gauges[[i]][,1]), gauges[[i]][,2], lwd=2)
            }
	    if(plotSnowpack){
		par(new=T)
